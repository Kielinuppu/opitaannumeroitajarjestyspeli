...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    if (!gameWrapper) {
        ...
        // Kutsu handleOrientation-funktiota vasta tässä
        handleOrientation();
    }
...
// Lisätään vielä yksi tarkistus window.onload-tapahtumaan
window.onload = function() {
    console.log("Window fully loaded");
    if (!gameWrapper) {
        gameWrapper = document.getElementById('game-wrapper');
        if (gameWrapper) {
            console.log("game-wrapper found in window.onload event");
            handleOrientation();
        } else {
            console.error("game-wrapper still not found in window.onload event");
        }
    }
}
